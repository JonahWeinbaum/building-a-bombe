...
\begin{center}
    ...
    thus we must have $P(A) = A$ as well in order for this to ultimately map back to $A$ and preserve our ciphertext-plaintext loop above.

    % In this diagram, we use upper case $A$, $B$, $C$ to denote the \textbf{outside line} -- that is, the mapping we are given from the plaintext-ciphertext pairing.
    % We know that when $A$ maps through the plugboard, followed by $\overline\sigma_1$, and finally back through the plugboard we get $B$. The lowercase letters $a$, $b$, $c$, $d$ denote the \textbf{inside line} -- that is, the possible plugboard 
    % mappings of each outside line. To wit, applying a current to the line $Aa$ (i.e. cable $A$ wire $a$) is supposing that when $A$ maps through the plugboard we will get $A$ as an output. In this way, by choosing where to apply current, we are implicitly
    % acting as the plugboard intermediating between the outside and inside lines.
    % \\\\Since following a line starting from the $A$ cable will show us how the line maps through $\overline\sigma = \overline\sigma_3\circ\overline\sigma_2\circ\overline\sigma_1$, 
    % in this diagram a closed loop of wires going around the diagram is analogous to a cycle in $\overline\sigma$. 
    % Thus, if we want to test a hypothesis that $P(A) = B$ we can apply a current to the outside line of $A$ and inside line of $b$ (i.e. $Ab$) and examine the deductions we get from this hypothesis. 
    % \begin{center}
    %     \begin{tikzpicture}[thick, scale=0.6, every node/.style={scale=0.7}]
    %         % Draw the box
    %         \draw[fill=lightgray] (2,-1.5) rectangle (4,2.5) node[midway] {};
    ...
    %         % Draw the wires entering the box
    %         \draw[-] (0, 2) -- (2, 2) node[midway, above] {a};
    %         \draw[-, red] (0, 1) -- (2, 1) node[midway, above] {b};
    %         \draw[-] (0, 0) -- (2, 0) node[midway, above] {c};
    %         \draw[-] (0,-1) -- (2,-1) node[midway, above] {d};
    ...
    %         % Draw the wires entering the box
    %         \draw[-] (0-3, 2-7) -- (2-3, 2-7) node[midway, above] {a};
    %         \draw[-, red] (0-3, 1-7) -- (2-3, 1-7) node[midway, above] {b};
    %         \draw[-] (0-3, 0-7) -- (2-3, 0-7) node[midway, above] {c};
    %         \draw[-] (0-3,-1-7) -- (2-3,-1-7) node[midway, above] {d};
    ...
    %         \draw[-] (4-3, 1-7) -- (6-3, 1-7) node[right, above] {b};
    %         \draw[-] (4-3, 0-7) -- (6-3, 0-7) node[right, above] {c};
    %         \draw[-, red] (4-3,-1-7) -- (6-3, -1-7) node[right, above] {d};
    ...
    %         \node at (3-3, -2-7) {$\overline\sigma_\beta$};
    ...
    % NOTE THAT EACH ENIGMA HAS THE SAME CYCLE TYPE