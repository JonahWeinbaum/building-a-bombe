...
struct VectorHash {
    size_t operator()(const tuple<vector<uint8_t>, vector<uint8_t>, int>& key) const {
        auto& [v1, v2, n] = key;
        size_t h1 = hash<int>()(n);
        size_t h2 = 0, h3 = 0;

        for (uint8_t x : v1) h2 ^= hash<uint8_t>()(x) + 0x9e3779b9 + (h2 << 6) + (h2 >> 2);
        for (uint8_t x : v2) h3 ^= hash<uint8_t>()(x) + 0x9e3779b9 + (h3 << 6) + (h3 >> 2);
        
        return h1 ^ h2 ^ h3;
    }
};

unordered_map<tuple<vector<uint8_t>, vector<uint8_t>, int>, double, VectorHash> memo;

struct VVIComparator {
    bool operator()(const vector<vector<uint8_t>>& v1, const vector<vector<uint8_t>>& v2) const {
...
set<vector<vector<uint8_t>>, VVIComparator> partition_of_partition(vector<uint8_t> p,
                                                    const vector<uint8_t> target_p, int depth = 0) {
...
int num_of_partition(const vector<uint8_t> &p, int n) {
  ...
  for (const auto &pair: p_m) {
    result /= pow(tgamma(pair.first + 1), pair.second)*tgamma(pair.second + 1); 
  }
  return (int)result;
}
...
int partitions_to_search_space(vector<uint8_t> &p1, vector<uint8_t> &p2, int n) {
  return num_of_cycle_type(p1, n) * num_of_cycle_type(p2, n);
}

vector<vector<vector<vector<uint8_t>>>> group_pops(set<vector<vector<uint8_t>>, VVIComparator> &pop1, set<vector<vector<uint8_t>>, VVIComparator> &pop2) {
...
float get_transitive_prob(vector<uint8_t> &p1, vector<uint8_t> &p2, int n) {
  ...
  for (auto &part: IPS[n-1]) {
    ...
    set<vector<vector<uint8_t>>, VVIComparator> c1_pop = partition_of_partition(p1, part);
    set<vector<vector<uint8_t>>, VVIComparator> c2_pop = partition_of_partition(p2, part);
    ...
    for (auto& group : groups) {
	  ...
	  subsubtotal *= partitions_to_search_space(group[i][0], group[i][1], part[i])
...
int main(void) {
  ...
  for (int k = 1; k <= n; k++) {
    ...
    float total = (IPS[k-1].size()*(IPS[k-1].size() + 1)) / 2;
    float step = 0.0;
    for (int i = 0; i < IPS[k-1].size(); i++) {
      for (int j = 0; j <= i; j++) {
      //xcout << step/total << endl;
