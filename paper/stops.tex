...
given by Turing himself in the \emph{Prof's Book}. Turing approaches this
...
as,a ``positions at which by altering the point at which the current
...
\end{center}
we would say this stop has a $13^2$ stop type. In general, if the orbits of a subgroup are sets $\omega = \{\omega_1,\dots,\omega_k\}$, we say that this represents a $1^{m_1^{(\omega)}}\dots n^{m_n^{(\omega)}}$ stop, where ${m_i^{(\omega)}}$ is the number of sets in $\omega$ with cardinality $i$. 
\\\\This is effectively
...
\begin{proof}
  ...
  \\\\Supposing a particular partition $\omega$ has partition type $1^{\ell_1}\dots n^{\ell_n}$, then our above argument can be framed as stating that there are
  \begin{center}
    $\prod_{i=1}^n{((i!)^2t_i)^{\ell_i}}$
  \end{center}
    permutations $x,y\in S_n$ such that $\langle x,y\rangle$ has an
  orbit structure equivalent to the partition type of $\omega$.
...
This repository can be found at \url{https://https://github.com/JonahWeinbaum/building-a-bombe}. This repository includes the scripts used for: 
\begin{itemize}
