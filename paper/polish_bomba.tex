...
% - Page 16 %
In the early 1930s, Polish cryptographer and mathematician Marian Rejewski had arguably the most difficult task in the process of breaking the Enigma. Not only did he need to determine a means to
...
\begin{align}
  \pi_1\pi_4 & =
  (\texttt{DVPFKXGZYO})(\texttt{EIJMUNQLHT})(\texttt{BC})(\texttt{RW})(\texttt{A})(\texttt{S})\label{char_1}
  \\
  \pi_2\pi_5 & =
  (\texttt{BLFQUEOUM})(\texttt{HJPSWIZRN})(\texttt{AXT})(\texttt{CGY})(\texttt{D})(\texttt{K})\label{char_2}
  \\
  \pi_3\pi_6 & = (\texttt{ABVIKTJGFCQNY})(\texttt{DUZREHLXWPSMO})\label{char_3}.
\end{align}
...
\begin{itemize}
  \item[(1)] A characteristic like $\pi_1\pi_4$, seen in \ref{char_1}, which has two
    singletons $\texttt{A}$ and $\texttt{S}$ must have that both $\pi_1$ and
    $\pi_4$ share the transposition $(\texttt{AS})$.
  \item[(2)] A characteristic like $\pi_3\pi_6$, seen in \ref{char_2}, (two disjoint 13
...
\subsection{Cillies}\label{section_cillies}
...
\begin{itemize}
  ...
  \item Letters forming trigrams on the Enigma keyboard such as
    ...
    % https://archive.org/details/enigmahowgermanm0000koza/page/240/mode/2up
    % PG 241 %
...
length to the cycle containing \texttt{A}. With this set strict of
...
Equipped with a means to determine each day's $\pi_i$s, Rejewski set
...
We now have deduced all of the rotor permutations $N$, $M$, $L$, $R$,
as well as each
...
pre-compute $N$, $P^{-1}NP$, $\dots$, $P^{4}NP^{-4}$ and arrange them
...
\]
\subsection{Recovering the Rotor Position and Order}
...
\subsection{Recovering the Ring Setting}\label{cyclometer_ring_setting}
...
key as in Section~\ref{characteristics}. He noticed that the cycle structure of a characteristic did not
...
jumping back and forth between $\pi_1$ and $\pi_4$ we we will
...
between $\pi_1$ and $\pi_4$ we will find alternating elements
of the pair of cycles of equal length which contain \texttt{A}. We
can mechanize this process via the following
circuit\footnote{Throughout this thesis, diagrams will represent a
...
($\pi_1\pi_3$, $\pi_2\pi_5$, and $\pi_3\pi_6$) as in Section~\ref{characteristics}, look up
...
%\footnote{Much like earlier cillies, some operators lazily
%   chose trigrams for their ground setting and message key which formed
% words like \texttt{BERLIN} or \texttt{LONDON}.}. % \begin{figure}[h]
%   \begin{center}
...
sequence, the cryptanalyst will never find enough messages with the same window
...
three messages with the following form seen below:
\[
  \setlength{\arraycolsep}{15pt}
  \begin{array}{c | c}
...
\begin{center}
  \scalebox{0.9} {
    \begin{circuitikz}
      ...
      \fill (1.75,2-1) ++(31.5:0.58) circle (2pt); % dot
      ...
      \fill (1.75,2-7) ++(31.5:0.58) circle (2pt); % dot
      ...
      % Circuit Completion
      \draw (3.5,3) to (8,3);
      \draw[fill=black] (3.5,3) circle (2pt); % dot
      \draw[fill=black] (3.5+1.5,3) circle (2pt); % dot
      \draw[fill=black] (3.5+3,3) circle (2pt); % dot
      \draw (8, -10) to (3.5, -10);
      \draw[purple] (-3.3, 3.3) rectangle (10.3, -10.3);
      \draw[fill=black](3.5,-10) circle (2pt); % dot
      \draw[fill=black] (3.5+1.5,-10) circle (2pt); % dot
      \draw[fill=black] (3.5+3,-10) circle (2pt); % dot
...
meant that finding keys with the Bomby could take nearly two hours~\cite[p.~242]{KozaczukEnigma1984}.
...
singleton cycle in $\pi_1\pi_4$ means that the letter contained
...
called {\bf{Zygalski sheets}}. Sheets produced in this thesis were constructed by code in our project's open-source repository~\cite{Weinbaum2023_building-a-bombe}. To our knowledge this is the first open-source code available for creating and using Zygalski sheets. 
\\\\To construct a sheet, we can set our
...
\begin{figure}[H]
  \begin{center}
    ...
    \end{tabular}}
  \end{center}
  \caption{(\texttt{B} - \texttt{I} \texttt{III} \texttt{II}) Zygalski sheet: \texttt{O} represents a hole and \texttt{.} represents its absence}
...
corresponds to sheet \texttt{(A - I III II)}. We
place it in the window and translate it so that its origin aligns
with (\texttt{A}, \texttt{A}). After doing this, the
holes in both \texttt{(B - I III II)} and \texttt{(A - I III II)}
will only line up at specific locations leading to a window with
holes with light shining through as follows,
\begin{figure}[H]
  \begin{center}
    \scalebox{0.8} {
      \begin{tabular}{c|cccccccccccccccccccccccccc}
...
\\\\Programmatically this can be dealt with by ensuring that all $4$
...
\begin{align*}
  \mathbb{P}(E_i) & = \dfrac{\dfrac{24!}{2^{12}12!}}{\dfrac{26!}{2^{13}13!}} \\
...
\begin{align*}
  ...
  \dfrac{\dfrac{22!}{2^{11}11!}}{\dfrac{26!}{2^{13}13!}}
  \\
...
% relationship between $\pi_{i}$ and $\pi_{i+3}$. At the time
